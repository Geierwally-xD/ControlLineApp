 ...
 ********************************************************************/	
    ...
    {
      Wire.begin();                             /* Initialize comuniaction */
     ...
     /* write configuration register */
      ...
      gyroThrottleServoSens_ = eePromStorage_.readServoThrottleSens(); /* read gyro throttle servo sensitivity from EEProm*/

      gyroAccelTimer_.Start();
      while(gyroAccelTimer_.IsExpired(20)==0)
      {} /* wait 20 ms */
 ...
 ********************************************************************/
  uint8_t GyroAccelSens::getGyroServoSensitivity(void) 
  {
    return(gyroThrottleServoSens_);
  }
 ...
 ********************************************************************/
    ...
    {  
      ...
      /* === Read gyroscope data === */
      elapsedTime = (float)gyroAccelTimer_.GetRelativeTickCount() / 1000.0;
      gyroAccelTimer_.Start();
      Wire.beginTransmission(device_Address_);
      Wire.write(GYRO_XOUT_H); /* Gyro data first register address 0x43 */
      Wire.endTransmission(false);
      Wire.requestFrom((int)device_Address_, (int)6, (int)true); /* Read 4 registers total, each axis value is stored in 2 registers */
...
/*********************************************************************
 * Method: void gyroTeach(uint8_t buttonPressState)
 *
 ...
 ********************************************************************/	
    void GyroAccelSens:: gyroTeach(uint8_t buttonPressState)
    {
      switch(gyroAccelTeachState_)
      {
        ...
        break;
        case gt_flightActive:        /* set gyro flight active / inactive */
        if(teachLEDTimer_.IsExpired(teachLEDTimeout_[gyroAccelExpoTeachIndex_]))
        {
          if(gyroAccelExpoTeachIndex_ < 2)
          {
            gyroAccelExpoTeachIndex_ ++;
            statusLed_->FlashLed(gyroAccelExpoTeachIndex_,100,200);
            teachLEDTimer_.Start();
        ...
        break;        
        case gt_servoSens:    /* gyro flight servo sensitivity from 0 = fast to 5 = slow*/
        if(teachLEDTimer_.IsExpired(teachLEDTimeout_[gyroAccelExpoTeachIndex_]))
        {
          if(gyroAccelExpoTeachIndex_ < 5)
          {
            gyroAccelExpoTeachIndex_ ++;
            if(gyroAccelExpoTeachIndex_ < 5)
            {
        ...
        { /* switch to teach state if button was pressed */
          switch(gyroAccelExpoTeachIndex_)
          {
            ...
            break;
            default:
            gyroThrottleServoSens_ = gyroAccelExpoTeachIndex_;
            break;
          }

          eePromStorage_.writeServoThrottleSens(gyroThrottleServoSens_);
          gyroAccelTeachState_ = gt_idle;
          expectedgyroAccelTeachState_ = gt_idle;
        }
        break;
